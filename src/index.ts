...
type OutputDirType = {
  ...
  types: { path: string; exist: ExistType };
};
...
const paths = (rootPath: string): Paths => {
  return {
    src: `path: path.join(${rootPath}, "src")`,
    component: { path: path.join(rootPath, "src", "components", "ui"), exist: undefined },
    icon: { path: path.join(rootPath, "src", "components", "ui", "icons"), exist: undefined },
    hook: { path: path.join(rootPath, "src", "hooks"), exist: undefined },
    tailwind: {
    ...
    types: [
      {
        name: "index",
        fileName: "index.d.ts",
        path: path.join(outputDir.types.path, "index.d.ts"), // /.../projectName/src/types/index.d.ts
        value: "// Add your types here\n\n export type Prettyfy<T> = {\n  [K in keyof T]: T[K];\n};\n",
      },
    ],
  } as Omit<LibraryType, "src">;
};
